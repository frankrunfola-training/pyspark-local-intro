...
cust_clean = customers_std.filter(~bad_rows_cust) #NOTE: can also use joins (anti-join).
...
txn_quarantine = txns.filter(bad_rows_txn)
txn_clean = txns.filter(~bad_rows_txn) #NOTE: can also use joins (anti-join).
